...
export function createCategoryLink(container, newsItemLimit, id) {
  ...
  catMore.setAttribute('class', 'news__link');
  ...
  catMoreLink.setAttribute('class', 'news__links')
  return catMoreLink;
 ...
 */
 export async function fetchAndRenderCategory(
  ...
  // Höfum fréttir! Ítrum og bætum við <ul>
    ...
    catNewsList.setAttribute('id', ulID)
    newSection.appendChild(catNewsList);

    for (let i = 0; i < Math.min(limit,nOnews); i += 1) {
      const newsSecRef = document.getElementById(ulID);
      const newsLinkWrapper = el('li');
      newsLinkWrapper.setAttribute('class', 'news__item');
      const newsLink = el('a', categoryNews.items[i].title);
      newsLink.setAttribute('class', 'news__link');
      newsLink.setAttribute('href', categoryNews.items[i].link);
      newsLinkWrapper.appendChild(newsLink);
      newsSecRef.appendChild(newsLinkWrapper);
    }
}
...
export async function fetchAndRenderLists(container, newsItemLimit) {
  ...
  for (let i = 0; i < newsItemLimit; i += 1){
    ...
    const link = createCategoryLink(container, newsItemLimit, catId);
    fetchAndRenderCategory(catId, thisSection, link, newsItemLimit);
  }
